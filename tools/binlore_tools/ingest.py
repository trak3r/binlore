...
from .vods import Vod, format_duration, get_yt_dlp_cmd, resolve_vod, run_yt_dlp, youtube_bot_cooldown_hint
...
def download_audio(vod: Vod, dest_dir: Path) -> Path:
    ...
    try:
        ...
        hint = f"\n{youtube_bot_cooldown_hint()}" if "youtu" in vod.url else ""
        raise RuntimeError(
